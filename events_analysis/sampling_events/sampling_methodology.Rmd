...
all_repo_releases_freq <- all_repo_releases_change %>%
  ...
  summarise(
    ...
    published_at_interval_log_min = min(published_at_interval_log),
    published_at_interval_log_max = max(published_at_interval_log)
  )  %>%
  mutate(
...
```{r}
repo_releases_time_between_freq_repo_summary <- repo_releases_time_between_freq_repo_summary %>%
  group_by(sample) %>%
  mutate(num_repos_log_max = max(num_repos_log))

repo_releases_time_between_freq_repo_summary_top <- repo_releases_time_between_freq_repo_summary %>%
  filter(num_repos_log == num_repos_log_max)

ggplot(data = repo_releases_time_between_freq_repo_summary_top, 
       aes(x=sample, y=num_repos, fill=num_releases_min_max)) +
  geom_bar(stat="identity", position="stack") + 
  ylab("Repos") +
  xlab("Sample") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(data = repo_releases_time_between_freq_repo_summary_top, 
       aes(x=sample, y=num_repos, fill=)) +
  geom_bar(stat="identity", position="stack") + 
  ylab("Repos") +
  xlab("Sample") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

#ggsave("repo_releases_time_between_freq_summary_top.png")
```

...
ggplot(data = all_repo_data_owner_freq, 
       aes(x=num_repos, y=factor(num_owners), fill=factor(num_repos))) +
  geom_bar(stat="identity") +
  theme(legend.position="none") +
...
```{r}
create_events_repo_summary <- readRDS("create_events_repo_summary.rds")
create_events_repo_type_summary <- readRDS("create_events_repo_type_summary.rds")
```
...
```{r}
create_repo_event_type_freq <- create_events_repo_type_summary %>%
  group_by(type, num_events_log) %>%
  summarise(
    num_events_max = max(num_events),
    num_events_min = min(num_events),
...
```{r}
ggplot(data = create_repo_event_type_freq, 
       aes(x=reorder(type, -num_repos), 
       y=num_repos,
       fill=factor(num_events_log))) +
  geom_bar(stat="identity", position="dodge") + 
...
ggsave("create_repo_event_type_freq.png")

create_repo_max_events <- create_repo_event_type_freq %>%
  filter(num_repos >= 10)

ggplot(data = create_repo_max_events, 
       aes(x=reorder(type, -num_repos), 
       y=num_repos, 
       fill=factor(num_events_max))) +
  geom_bar(stat="identity", position="dodge") + 
...
ggsave("create_repo_max_events.png")
```
...
```{r}
create_repo_actors_freq <- create_events_repo_summary %>%
  group_by(total_actors_log) %>%
  summarise(
    total_actors_max = max(total_actors),
    total_actors_min = min(total_actors),
...
```{r}
ggplot(data = create_repo_actors_freq, 
       aes(x = factor(total_actors_max), 
           y = num_repos,
           fill=factor(total_actors_log))) +
  geom_bar(stat="identity", position="stack") +
...
ggsave("create_repo_actors_freq.png")
```

### Actors per Event Type per Repo

```{r}
create_repo_type_actors_freq <- create_events_repo_type_summary %>%
  group_by(type, num_actors_log) %>%
  summarise(
    num_actors_max = max(num_actors),
    num_actors_min = min(num_actors),
...
```{r}
ggplot(data = create_repo_type_actors_freq, 
       aes(x = type, 
           y = num_repos,
           fill=factor(num_actors_log))) +
  geom_bar(stat="identity", position="dodge") +
...
ggsave("create_repo_event_type_actors_freq.png")

create_repo_max_actors <- create_repo_type_actors_freq %>%
  filter(num_repos >= 10)

ggplot(data = create_repo_max_actors, 
       aes(x=reorder(type, -num_repos), 
       y=num_repos, 
       fill=factor(num_actors_max))) +
  geom_bar(stat="identity", position="dodge") + 
...
ggsave("create_repo_max_actors.png")
...
```{r}
create_repo_e2a_pct_freq <- create_events_repo_summary %>%
  group_by(events_per_actor_pct_log) %>%
  summarise(
    events_per_actor_pct_max = max(events_per_actor_pct),
    events_per_actor_pct_min = min(events_per_actor_pct),
...
ggplot(data = create_repo_e2a_pct_freq, 
       aes(x = factor(total_actors_max), 
           y = num_repos,
           fill=factor(events_per_actor_pct_max_rnd))) +
  geom_bar(stat="identity", position="dodge") +
...
ggsave("create_repo_e2a_pct_freq.png")
